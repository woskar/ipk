...
std::unordered_map<std::string,int> get_word_frequencies()
{
    unsigned int count = 0;
    ...
    std::cout << "Total of " << count << " words included." << std::endl;
    return dictionary;
}
...
void print_word_frequencies(const std::unordered_map<std::string, int>& frequencies)
{
...
void wordfrequencies(const std::unordered_map<std::string,int>& dict)
{
    unsigned int count = 0;
...
int main(int argc, char** argv)
{
    ...
    // dieser Teil aktiviert für Teil a), die Buchstaben
    ...
    // dieser Teil aktiviert für Teil b), die Wörter
    std::unordered_map<std::string, int> dict = get_word_frequencies();
